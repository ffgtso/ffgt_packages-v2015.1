...
function action_reboot()
  ...
  local location
  location = uci:get_first("gluon-node-info", "location", "latitude") .. " " .. uci:get_first("gluon-node-info", "location", "longitude")
  ...
  if nixio.fork() ~= 0 then
    luci.template.render("gluon-config-mode/reboot",
      {luci=luci, pubkey=pubkey, hostname=hostname, site=site, sysconfig=sysconfig, location=location})
  else
